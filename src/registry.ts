...
export const ACTOR_ROLES: readonly ActorRole[] = ['master', 'colleague', 'customer', 'stranger', 'blocked']
...
export function actorHome(): string {
...
export function loadRegistry(): ActorStore {
  const path = registryPath()
  if (!existsSync(path)) return { revision: 0, entities: [] }
  try {
    const store = JSON.parse(readFileSync(path, 'utf8')) as ActorStore
    if (!Array.isArray(store.entities)) return { revision: 0, entities: [] }
...
export function saveRegistry(store: ActorStore): void {
  const path = registryPath()
...
export function resolveAlias(store: ActorStore, entity: ActorEntity): ActorEntity {
  let current = entity
  for (let hop = 0; hop < 8; hop++) {
    if (current.mergedInto === undefined) return current
    const next = store.entities.find(e => e.id === current.mergedInto)
    if (next === undefined) return current // 目标丢失：停在原地，fail-closed
    current = next
  }
  return current
}
...
export function findByChannelIdentity(store: ActorStore, channel: string, userId: string): ActorEntity | undefined {
  const ch = normalizeChannel(channel)
  const uid = normalizeUserId(userId)
  if (ch === '' || uid === '') return undefined
  const hit = store.entities.find(e =>
 ...
 */
export function provision(channel: unknown, userId: unknown, displayName?: unknown): ProvisionResult {
  const ch = normalizeChannel(channel)
  const uid = normalizeUserId(userId)
  if (ch === '' || uid === '') {
  ...
  if (existing !== undefined) return { entity: resolveAlias(store, existing), created: false }
...
export function resolve(channel: unknown, userId: unknown): ActorEntity | null {
  const store = loadRegistry()
...
export function getEntity(entityId: string): ActorEntity | null {
  const store = loadRegistry()
 ...
 */
export function bindMaster(channel: unknown, userId: unknown): BindMasterResult {
...
export function setRole(entityId: string, role: ActorRole): SetRoleResult {
  if (!ACTOR_ROLES.includes(role)) return { ok: false, error: `未知角色：${String(role)}` }
  const store = loadRegistry()
  const entity = store.entities.find(e => e.id === entityId && e.mergedInto === undefined)
  if (entity === undefined) return { ok: false, error: '实体不存在' }
  if (role === 'master') {
    const currentMaster = store.entities.find(e => e.mergedInto === undefined && e.role === 'master' && e.id !== entityId)
    if (currentMaster !== undefined) currentMaster.role = 'colleague'
  }
  entity.role = role
  saveRegistry(store)
  return { ok: true, entity }
...
export function addBinding(entityId: string, channel: unknown, userId: unknown): SetRoleResult {
  const ch = normalizeChannel(channel)
  const uid = normalizeUserId(userId)
  if (ch === '' || uid === '') return { ok: false, error: 'channel/userId 不能为空' }
  const store = loadRegistry()
  const entity = store.entities.find(e => e.id === entityId && e.mergedInto === undefined)
  if (entity === undefined) return { ok: false, error: '实体不存在' }
 ...
 */
export function merge(sourceId: string, targetId: string): MergeResult {
  if (sourceId === targetId) return { ok: false, error: '不能归并到自身' }
  const store = loadRegistry()
  ...
  if (source === undefined || target === undefined) return { ok: false, error: '实体不存在' }
...
export function list(filter: ActorListFilter = {}): ActorEntity[] {
  const store = loadRegistry()
...
export function stats(): { total: number; byRole: Record<ActorRole, number>; revision: number } {
  const store = loadRegistry()
  const byRole = { master: 0, colleague: 0, customer: 0, stranger: 0, blocked: 0 } as Record<ActorRole, number>
  for (const e of store.entities) {
    if (e.mergedInto !== undefined) continue
